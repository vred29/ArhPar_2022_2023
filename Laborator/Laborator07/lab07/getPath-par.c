...
Task makeTask(int* partialPath, int step, int dest) 
{
	Task task;
	myData* aux =(myData*)malloc(sizeof(myData));

	aux->partialPath = malloc(sizeof(int) * 100);
	memcpy ( aux->partialPath, partialPath, sizeof(int)*100 );
	aux->step=step;
	aux->dest=dest;
...
void getPath(void* data) {
	int *partialPath = ((myData*)data)->partialPath;
	int step = ((myData*)data)->step;
	int destination = ((myData*)data)->dest;
	...
	for (int i = 0; i < numEdges; i++) {
		if (graph[i][0] == lastNodeInPath) {
			...
			putTask(makeTask(newPartialPath, step + 1, destination));
		}
	}
}
...
int main(int argc, char** argv) 
{
	...
	startWorkers();
	putTask(makeTask(partialPath, 1, destinationVertex));
	joinWorkerThreads();
	return 0;
}