...
Task makeTask(int* colors, int step) 
{
	Task task;
	myDataColor* aux =(myDataColor*)malloc(sizeof(myDataColor));

	aux->colors = (int*) malloc(sizeof(int)*numVertices);
	memcpy ( aux->colors, colors, sizeof(int)*numVertices  );
	aux->step=step;
...
void colorGraph(void* data) {
	int* colors = ((myDataColor*)data)->colors;
	int step = ((myDataColor*)data)->step;
	if (step == numVertices) {
		print(colors, step);
	...
	for (int i = 0; i < COLORS; i++) {
		...
		if (verifyColors(newColors, step))
		{
			...
			putTask(makeTask(newColors, step + 1));
		}
	}
}
