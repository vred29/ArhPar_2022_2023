...
int main(int argc, char **argv)
{
	...
	cl_kernel kernel = getAndCompileKernel("ex07.cl", "oets_more_wg", context, deviceid);
	...
	// Execute kernel
	size_t total_work_size = vectorLen/2;
	...
	printf("Vector length is N = %d\n", vectorLen);
	//apelez de mai multe ori functia
	for(unsigned int i=0;i<vectorLen;i++){
		ret = clSetKernelArg(kernel, 3, sizeof(unsigned int), &i);
		handleError(ret, __LINE__, __FILE__);
