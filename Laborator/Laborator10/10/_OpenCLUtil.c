...
cl_device_id initOpenCL(cl_context *context, cl_command_queue *commandQueue)
{
	...
	// aici calculez max compute units
	ret = clGetDeviceInfo(device_ids[selectedDevice], CL_DEVICE_MAX_COMPUTE_UNITS, 0, NULL, &param_value_size);
	handleError(ret, __LINE__, __FILE__);

	cl_uint deviceMax;
	ret = clGetDeviceInfo(device_ids[selectedDevice], CL_DEVICE_MAX_COMPUTE_UNITS, param_value_size, &deviceMax, &param_value_size);
	handleError(ret, __LINE__, __FILE__);
	printf("DEBUG: Max compute units %d\n", (int)deviceMax);
